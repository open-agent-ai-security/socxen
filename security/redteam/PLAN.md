...
- **Model.** Gate on the **weakest supported model** (currently **Sonnet**, which *is* a supported model).
  If the dumbest model we support resists, stronger ones do too, so this is the conservative gate. Sweep
  Opus for extra signal; **Haiku is not a supported model.**
...
2. **Model** — gate on the **weakest supported model** (currently **Sonnet** — supported, not merely a
   surfacing model). If the dumbest supported model resists, stronger ones do too. Sweep Opus for extra
   signal; **Haiku is not supported.**

...
| `tools_list` | the remote's tool definitions arrive (once per session) | `tool_count`, `metadata_stripped` / `metadata_flagged` (hidden code points the metadata screen removed or flagged in descriptions and schema text — counts only), `metadata_screen_failed`, `odd_names` (tool names carrying a hidden code point, reported not altered), `unclassified_tools` (names no tier classifies; treated as writes) |
